...
export class AppComponent {
  ...
  types(){
    ...
    const PRUEBA: string = "prueba constante";
    let pruebaDos: string = "prueba variable";

    console.log(PRUEBA, pruebaDos);
  }
  ...
  tryCatchStatement(){
    ...
    try {
      throw "Se presento un error"; //Forzo/lanzo una exception
    } catch (e) {
      console.log(e);
    }
  ...
  basicFunctions(){
    ...
    function multiplyNumbers(a:number, b:number, c?: number): number{
      return a * b * (c ?? 1); // "??" si no hay un valor asigando, asignale 1
    }
